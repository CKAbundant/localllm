...
class GenAnalysis:
    ...
    def single_senti(
        self, news_list: list[dict[str, int]], temp_path: str, start_idx: int
    ) -> None:
    ...
    def batch_senti(
        ...
        with open(temp_path, "a", newline="") as csvfile:
            ...
            for idx in tqdm(range(0, len(updated_list), self.batch_size)):
                ...
                # Generate 'ratings' dictionary containing 'id', 'rating' and 'reasons'
                ratings_list = self.local_llm.batch_senti_rate(
                    batch_news, self.batch_size
                )
    ...
    def gen_dataset(self) -> pd.DataFrame:
        ...
        if self.batch_size is None:
            self.single_senti(news_list, temp_path, start_idx)
        else:
            self.batch_senti(news_list, temp_path, start_idx)

...
class GenAnalysis:
    ...
    Args:
        ...
        batch_size (int):
            Number of news items to rate sentiment in single batch (Default: 20).
    ...
    def run_batch(
        ...
        Args:
            batch_size (int):
                If provided, number of news items to rate sentiment in single batch.
        ...
        df_senti = self.batch_gen_dataset(batch_size)
    ...
    def gen_batch_dataset(self, batch_size: int) -> pd.DataFrame:
        ...
        with open(temp_path, "a", newline="") as csvfile:
            ...
            for idx in range(0, len(updated_list), batch_size):
                # Generate batch of news items of size 'batch_size'
                batch_news = updated_list[idx : idx + batch_size]

                # Generate 'ratings' dictionary containing 'id', 'rating' and 'reasons'
                ratings_list = self.local_llm.batch_senti_rate(batch_news, batch_size)

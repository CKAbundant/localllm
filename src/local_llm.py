...
class InferLLM(ABC):
    ...
    def senti_rate(self, news_item: dict[str, int | str]) -> dict[str, Any]:
        ...
        while counter < 3:
            try:
                ...
                # Expected only 1 ratings dictionary for 1 news item
                payload = self.gen_response(content, expected_num=1)
    ...
    def batch_senti_rate(
        ...
        while counter < 3:
            try:
                ...
                # Expected length of payload to be the same as length of 'news_batch'
                payload = self.gen_response(content, expected_num=len(news_batch))
    ...
    def gen_response(
        self, content: str, expected_num: int
    ) -> list[dict[str, int | str]]:
        """Generate list of ratings generated from local LLM.

        Args:
            content (str):
                String content generated by local llm.
            expected_num (int):
                Number of items expected. If payload is a dictionary, then
                'expected_num' = 1.

        Returns:
            ratings_list = (list[dict[str, int | str]]):
                List of 'ratings' i.e. dictionaries containing 'id', 'rating'
                and 'reasons'.
        """
        ...
        # Convert to either dictionary or list of dictionary
        payload = ast.literal_eval(content)
        print(f"\npayload : \n\n{payload}\n")
        print(f"type(payload) : {type(payload)}")

        if not isinstance(payload, (list, dict)):
            raise ValueError(f"payload is neither list or dictionary.")

        if isinstance(payload, list) and len(payload != expected_num):
            raise ValueError(
                f"Length of payload ({len(payload)} != expected length ({expected_num}))"
            )

        if isinstance(payload, dict):
            # Convert from dictionary to list of dictionary
            return [payload]

        # payload is already list of dictionary
        return payload
